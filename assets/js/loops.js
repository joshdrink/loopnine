album = {
    aaWallace: {
        name: 'unreleased',
        art: 'AAWallace3.jpeg'
    },
    aatwlwm: {
        name: 'Volume One',
        art: 'aatwlwm.jpg'
    },
    thePhantomMoves: {
        name: 'The Phantom Moves',
        art: 'Alert The Medic.jpg'
    },
    soLongAtTheFair: {
        name: 'So Long At The Fair',
        art: 'Anne Janelle.jpg'
    },
    nightmareSingle: {
        name: 'Nightmare (Single)',
        art: 'Bloody Diamonds.jpg'
    },
    cannon: {
        name: 'Cannon',
        art: 'Cam Smith.jpg'
    },
    youngBlood: {
        name: 'Young Blood',
        art: 'Chris Martin.jpg'
    },
    doItAllAgainSingle: {
        name: 'Do It All Again (Single)',
        art: 'Dylan Guthro.jpg'
    },
    brokenDownHorse: {
        name: 'Broken Down Horse',
        art: 'E.B. Anderson.jpg'
    },
    soLongEP: {
        name: 'So Long EP',
        art: 'Glory Glory.jpg'
    },
    conversationalist: {
        name: 'Conversationalist',
        art: 'In-Flight Safety.jpg'
    },
    decisions: {
        name: 'Act II: Decisions',
        art: 'Jessie Brown.jpg'
    },
    theGreatUpheavel: {
        name: 'The Great Upheavel',
        art: 'kuato.jpg'
    },
    handsSingle: {
        name: 'Hands (Single)',
        art: 'Like A Motorcycle.jpg'
    },
    yourLastWar: {
        name: 'Your Last War',
        art: 'Heather Green.png'
    },
    mineForTheSummertimeSingle: {
        name: 'Mine For The Summertime',
        art: 'Kyle Mischiek.jpg'
    },
    maryBethDeSceneAlbum:{
        name: 'Mary Beth de Scène',
        art: 'Mary Beth de Scène.JPG'
    },
    summertimeWinSingle:{
        name: 'Summertime Win (Single)',
        art: 'Quake Matthews.jpg'
    },
    ephemeral: {
        name: 'Ephemeral',
        art: 'Rich Aucoin.jpg'
    }
};
artist = {
    aaWallace: {
        name: 'AA Wallace',
        twitter: 'aaontour',
        web: 'www.aawallace.xxx',
        albums: [
            album.aaWallace
        ]
    },
    aatwlwm: {
        name: 'And All That Were Left Were Monsters',
        twitter: 'aatwlwm',
        web: 'andallthatwereleftweremonsters.com',
        albums: [
            album.aatwlwm
        ]
    },
    alertTheMedic: {
        name: 'Alert The Medic',
        twitter: 'alertthemedic',
        web: 'www.alertthemedic.com',
        albums: [
            album.thePhantomMoves
        ]
    },
    annaJanelle: {
        name: 'Anne Janelle',
        twitter: 'annEjanelleMus',
        web: 'www.annejanelle.com',
        albums: [
            album.soLongAtTheFair
        ]
    },
    bloodyDiamonds: {
        name: 'Bloody Diamonds',
        twitter: 'bloodydiamonds_',
        web: 'www.bloodydiamonds.com',
        albums: [
            album.nightmareSingle
        ]
    },
    camSmith: {
        name: 'Cam Smith Ft. Nicole Ariana',
        twitter: 'CamSmithMusic',
        web: 'camsmithmusic.com',
        albums: [
            album.cannon
        ]
    },
    chrisMartinTrio: {
        name: 'Chris Martin Trio',
        twitter: 'ChrisMartinTrio',
        web: 'chrismartintrio.com',
        albums: [
            album.youngBlood
        ]
    },
    dylanGuthro: {
        name: 'Dylan Guthro',
        twitter: 'DylanGuthro',
        web: 'www.dylanguthro.com',
        albums: [
            album.doItAllAgainSingle
        ]
    },
    ebAnderson: {
        name: 'E.B. Anderson & The Resolutes',
        twitter: 'EBAResolutes',
        web: 'www.ebaresolutes.com',
        albums: [
            album.brokenDownHorse
        ]
    },
    gloryGlory: {
        name: 'Glory Glory',
        twitter: 'glorygloryband',
        web: 'www.glorygloryband.com',
        albums: [
            album.soLongEP
        ]
    },
    inFlightSafety: {
        name: 'In-Flight Safety',
        twitter: 'InFlight_Safety',
        web: 'www.inflightsafety.ca',
        albums: [
            album.conversationalist
        ]
    },
    jessieBrown: {
        name: 'Jessie Brown & The Black Divine',
        twitter: 'jessie_brown_',
        web: 'jessiebrownband.com',
        albums: [
            album.decisions
        ]
    },
    kuato: {
        name: 'Kuato',
        twitter: 'KuatoHalifax',
        web: 'kuatoband.com',
        albums: [
            album.theGreatUpheavel
        ]
    },
    likeAMotorcycle: {
        name: 'Like A Motorcycle',
        twitter: 'LikeAMotorcycle',
        web: 'likeamotorcycle.com',
        albums: [
            album.handsSingle
        ]
    },
    heatherGreen: {
        name: 'HeatherGreen',
        twitter: '_heathergreen',
        web: 'www.heathergreen.ca',
        albums: [
            album.yourLastWar
        ]
    },
    kyleMischiek: {
        name: 'Kyle Mischiek',
        twitter: 'KyleMischiek',
        web: 'kylemishiek.com',
        albums: [
            album.mineForTheSummertimeSingle
        ]
    },
    maryBethDeScene: {
        name: 'Mary Beth de Scène',
        twitter: 'MaryBethdeScene',
        web: 'www.marybethdescene.com',
        albums: [
            album.maryBethDeSceneAlbum
        ]
    },
    quake: {
        name: 'Quake ft. Dylan Guthro',
        twitter: 'QuakeMatthews',
        web: 'www.youtube.com/quakemusic',
        albums: [
            album.summertimeWinSingle
        ]
    },
    richAucoin: {
        name: 'Rich Aucoin',
        twitter: 'richaucoin',
        web: 'richaucoin.ca',
        albums: [
            album.ephemeral
        ]
    }
};
song = {
    vltGirls: {
        name: 'VLT Girls',
        album: album.aaWallace,
        artist: artist.aaWallace
    },
    graceComeBack: {
        name: 'Grace Come Back',
        album: album.thePhantomMoves,
        artist: artist.alertTheMedic
    },
    blackIsTheColour: {
        name: 'Black Is The Colour',
        album: album.soLongAtTheFair,
        artist: artist.annaJanelle
    },
    nightmare: {
        name: 'Nightmare',
        album: album.nightmareSingle,
        artist: artist.bloodyDiamonds
    },
    feelTheSame: {
        name: 'Feel The Same',
        album: album.cannon,
        artist: artist.camSmith
    },
    strongIntuition: {
        name: 'Strong Intuition',
        album: album.youngBlood,
        artist: artist.chrisMartinTrio
    },
    doItAllAgain: {
        name: 'Do It All Again',
        album: album.doItAllAgainSingle,
        artist: artist.dylanGuthro
    },
    moveYourFeet: {
        name: 'Move your Feet',
        album: album.brokenDownHorse,
        artist: artist.ebAnderson
    },
    takeMyTime: {
        name: 'Take My Time',
        album: album.soLongEP,
        artist: artist.gloryGlory
    },
    indigoSon: {
        name: 'Indigo Son',
        album: album.soLongEP,
        artist: artist.gloryGlory
    },
    stockholm: {
        name: 'Stockholm',
        album: album.conversationalist,
        artist: artist.inFlightSafety
    },
    spite: {
        name: 'Spite',
        album: album.decisions,
        artist: artist.jessieBrown
    },
    redSand: {
        name: 'Red Sand',
        album: album.theGreatUpheavel,
        artist: artist.kuato
    },
    hands: {
        name: 'Hands',
        album: album.handsSingle,
        artist: artist.likeAMotorcycle
    },
    dayIWasBorn: {
        name: 'Day I Was Born',
        album: album.yourLastWar,
        artist: artist.heatherGreen
    },
    mineForTheSummertime: {
        name: 'Mine For The Summertime',
        album: album.mineForTheSummertimeSingle,
        artist: artist.kyleMischiek
    },
    henri: {
        name: 'Henri',
        album: album.maryBethDeSceneAlbum,
        artist: artist.maryBethDeScene
    },
    summertimeWin: {
        name: 'Henri',
        album: album.summertimeWinSingle,
        artist: artist.quake
    },
    wantToBelieve: {
        name: 'Want to Believe',
        album: album.ephemeral,
        artist: artist.richAucoin
    },
    godComplex: {
        name: 'God Complex',
        album: album.aatwlwm,
        artist: artist.aatwlwm
    }
};

loopList = {
    vltSynth: {
        name: 'vltSynth',
        song: song.vltGirls,
        url: 'aa_wallace-vlt_girls-synth.wav'
    },
    vltDrums: {
        name: 'vltDrums',
        song: song.vltGirls,
        url: 'aa_wallace-vlt_girls-drums.wav'
    },
    graceComeBackDrums: {
        name: 'graceComeBackDrums',
        song: song.graceComeBack,
        url: 'alert_the_medic-grace_come_back-drums.wav'
    },
    graceComeBackGuitar: {
        name: 'graceComeBackGuitar',
        song: song.graceComeBack,
        url: 'alert_the_medic-grace_come_back-guitar.wav'
    },
    godComplexDrums: {
        name: 'godComplexDrums',
        song: song.godComplex,
        url: 'and_all_that_were_left_were_monsters-god_complex-drums.wav'
    },
    godComplexSynth: {
        name: 'godComplexSynth',
        song: song.godComplex,
        url: 'and_all_that_were_left_were_monsters-god_complex-guitar.wav'
    },
    blackIsTheNewColourDrums: {
        name: 'blackIsTheNewColourDrums',
        song: song.blackIsTheColour,
        url: 'anne_janelle-black_is_the_new_colour-drums.wav'
    },
    blackIsTheNewColourSynth:{
        name: 'blackIsTheNewColourSynth',
        song: song.blackIsTheColour,
        url: 'anne_janelle-black_is_the_new_colour-drums.wav'
    },
    nightmareKeys: {
        name: 'nightmareKeys',
        song: song.nightmare,
        url: 'bloody_diamonds-nightmare-keys.wav'
    },

    feelTheSameVocals: {
        name: 'feelTheSameVocals',
        song: song.feelTheSame,
        url: 'cam_smith-feel_the_same-vocals.wav'
    },
    strongIntuitionGuitar: {
        name: 'strongIntuitionGuitar',
        song: song.strongIntuition,
        url: 'chris_martin-strong_intuition-guitar.wav'
    },
    doItAllAgainBass: {
        name: 'doItAllAgainBass',
        song: song.doItAllAgain,
        url: 'dylan_guthro-do_it_all_again-bass.wav'
    },
    doItAllAgainDrums: {
        name: 'doItAllAgainDrums',
        song: song.doItAllAgain,
        url: 'dylan_guthro-do_it_all_again-drums.wav'
    },
    doItAllAgainDrums2: {
        name: 'doItAllAgainDrums2',
        song: song.doItAllAgain,
        url: 'dylan_guthro-do_it_all_again-drums.wav'
    },
    doItAllAgainGuitar: {
        name: 'doItAllAgainGuitar',
        song: song.doItAllAgain,
        url: 'dylan_guthro-do_it_all_again-guitar.wav'
    },
    doItAllAgainHorns: {
        name: 'doItAllAgainHorns',
        song: song.doItAllAgain,
        url: 'dylan_guthro-do_it_all_again-horns.wav'
    },
    moveYourFeetKeys: {
        name: 'moveYourFeetKeys',
        song: song.moveYourFeet,
        url: 'eb_anderson-move_your_feet-keys.wav'
    },
    indigoSonDrums: {
        name: 'indigoSonDrums',
        song: song.indigoSon,
        url: 'glory_glory-indigo_son-drums.wav'
    },
    indigoSonSynth: {
        name: 'indigoSonSynth',
        song: song.indigoSon,
        url: 'glory_glory-indigo_son-synth.wav'
    },
    takeMyTimeBass: {
        name: 'takeMyTimeBass',
        song: song.takeMyTime,
        url: 'glory_glory-take_my_time-bass.wav'
    },
    takeMyTimeGuitar: {
        name: 'takeMyTimeGuitar',
        song: song.takeMyTime,
        url: 'glory_glory-take_my_time-guitar.wav'
    },
    dayIWasBornDrums: {
        name: 'dayIWasBornDrums',
        song: song.dayIWasBorn,
        url: 'heather_green-day_i_was_born-drums.wav'
    },
    dayIWasBornPiano: {
        name: 'dayIWasBornPiano',
        song: song.dayIWasBorn,
        url: 'heather_green-day_i_was_born-piano.wav'
    },
    dayIWasBornRhodes: {
        name: 'dayIWasBornRhodes',
        song: song.dayIWasBorn,
        url: 'heather_green-day_i_was_born-rhodes.wav'
    },
    stockholmDrums: {
        name: 'stockholmDrums',
        song: song.stockholm,
        url: 'in_flight_safety-stockholm-drums.wav'
    },
    stockholmGuitar: {
        name: 'stockholmGuitar',
        song: song.stockholm,
        url: 'in_flight_safety-stockholm-guitar.wav'
    },
    stockholmSynth: {
        name: 'stockholmSynth',
        song: song.stockholm,
        url: 'in_flight_safety-stockholm-guitar.wav'
    },
    spiteGuitar: {
        name: 'spiteGuitar',
        song: song.spite,
        url: 'jessie_brown-spite-guitar.wav'
    },
    wantToBelieveDrums3: {
        name: 'wantToBelieveDrums3',
        song: song.wantToBelieve,
        url: 'rich_aucoin-want_to_believe_drums_3.wav'
    },
    wantToBelieveDrums2: {
        name: 'wantToBelieveDrums2',
        song: song.wantToBelieve,
        url: 'rich_aucoin-want_to_believe_drums_2.wav'
    },
    wantToBelieveDrums: {
        name: 'wantToBelieveDrums',
        song: song.wantToBelieve,
        url: 'rich_aucoin-want_to_believe_drums.wav'
    },
    summertimeWinSynth: {
        name: 'summertimeWinSynth',
        song: song.summertimeWin,
        url: 'quake_matthews-summertime_win-synth.wav'
    },
    summertimeWinDrums: {
        name: 'summertimeWinDrums',
        song: song.summertimeWin,
        url: 'quake_matthews-summertime_win-drums.wav'
    },
    henriAccordion: {
        name: 'henriAccordion',
        song: song.henri,
        url: 'mary_beth_de_scene-henri-accordion.wav'
    },
    handsDrums: {
        name: 'handsDrums',
        song: song.hands,
        url: 'like_a_motorcycle-hands-drums.wav'
    },
    handsBass: {
        name: 'handsBass',
        song: song.hands,
        url: 'like_a_motorcycle-hands-bass.wav'
    },
    mineForTheSummertimeSynth: {
        name: 'mineForTheSummertimeSynth',
        song: song.mineForTheSummertime,
        url: 'kyle_mischiek-mine_for_the_summertime-synth.wav'
    },
    mineForTheSummertimeDrums: {
        name: 'mineForTheSummertimeDrums',
        song: song.mineForTheSummertime,
        url: 'kyle_mischiek-mine_for_the_summertime-drums.wav'
    },
    redSandGuitar: {
        name: 'redSandGuitar',
        song: song.redSand,
        url: 'kuato-red_sand-guitar.wav'
    },
    redSandDrums2: {
        name: 'redSandDrums2',
        song: song.redSand,
        url: 'kuato-red_sand-drums_2.wav'
    },
    redSandDrums: {
        name: 'redSandDrums',
        song: song.redSand,
        url: 'kuato-red_sand-drums.wav'
    }
}

groupA = [loopList.blackIsTheNewColourSynth, loopList.dayIWasBornPiano, loopList.dayIWasBornRhodes, loopList.doItAllAgainBass, loopList.doItAllAgainGuitar, loopList.doItAllAgainHorns, loopList.feelTheSameVocals, loopList.godComplexSynth, loopList.graceComeBackGuitar, loopList.handsBass, loopList.henriAccordion, loopList.indigoSonSynth, loopList.mineForTheSummertimeSynth, loopList.moveYourFeetKeys, loopList.nightmareKeys, loopList.redSandGuitar, loopList.spiteGuitar, loopList.stockholmGuitar, loopList.stockholmSynth, loopList.strongIntuitionGuitar, loopList.summertimeWinSynth, loopList.takeMyTimeGuitar, loopList.vltSynth];
groupB = [loopList.blackIsTheNewColourDrums, loopList.dayIWasBornDrums, loopList.doItAllAgainDrums, loopList.doItAllAgainDrums2, loopList.godComplexDrums, loopList.graceComeBackDrums, loopList.handsDrums, loopList.indigoSonDrums, loopList.mineForTheSummertimeDrums, loopList.redSandDrums, loopList.redSandDrums2, loopList.stockholmDrums, loopList.summertimeWinDrums, loopList.vltDrums, loopList.wantToBelieveDrums, loopList.wantToBelieveDrums2, loopList.wantToBelieveDrums3];

groupALength = 23;
groupBLength = 17;